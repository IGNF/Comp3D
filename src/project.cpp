        ...
        for (unsigned i=0;i<selectedPoints.size()-1;i++)
        {
            ...
            for (unsigned j=i+1;j<selectedPoints.size();j++)
            {
                ...
                if ((ptA->dimension>=2)&&(ptB->dimension>=2))
                {
                    ...
                    tdouble azimAB = atan2(dx, dy) + Project::theone()->projection.getStereoMeridianConvergence(ptA->coord_comp);
                    if (azimAB < 0){
                        azimAB += 2*PI;
                    }
                    azimAB = fromRad(azimAB,Project::theone()->config.filesUnit);
                    ...
                    tdouble sigma_azim = fromRad(lsquares.sigma_0*sqrt(sqr(dx)*(vy1+vy2-2*cy1y2)+sqr(dy)*(vx1+vx2-2*cx1x2)+2*dx*dy*(cx1y2+cx2y1-cx1y1-cx2y2))/sqr(distAB),
                                                 Project::theone()->config.filesUnit);
