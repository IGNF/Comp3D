...
//  if ((data["config"]["compute_type"] == 0) && (data["computation"]["compensationDone"] || data["computation"]["asked_to_stop"])) {
  ...
  if (data["computation"]["computation_start"]!=="not-a-date-time"){
     items.push("<dt>" + _("Début du calcul") + " :</dt><dd>" + data["computation"]["computation_start"] + '</dd>');
     items.push("<dt>" + _("Durée du calcul") + " :</dt><dd>" + data["computation"]["computation_duration"] +'</dd>');
  }
...
function show_coord_init(){
  ...
  for (var i = 0; i < all_pts.length; i++) {
    var pt = all_pts[i];
    items.push('<tr class=\"' + parite[i % 2] + '\"><td class=\"name\">' + pt.name + add_post(codes_pt[pt.code]) +
...
function obs2tab(obs,i,isbiggest)
{
    ...
    str='<tr  class=\"' + parite[i % 2] + str_active_class + '\" id="' + all_files[obs.file] + ':' + obs.line + '" title="' + all_files[obs.file] + ':' + obs.line +'">';
    if (isbiggest)
        str+='<td><a href="#' + all_files[all_active_obs[i].file] + ':' + all_active_obs[i].line + '">↑</a>';

    str+='<td class=\"name\">' + str_etoile + codes_obs[obs.code] +
            '</td><td class=\"name\">' + obs.from + add_post(codes_pt[all_pts_codes[obs.from]]) +
            '</td><td class=\"name\">' + obs.to + add_post(codes_pt[all_pts_codes[obs.to]]) +
            '</td><td>' + parseFloat(obs.original_value).toFixed(num_decimales) + add_post(obs.unit_str) +
            '</td><td>' + parseFloat(obs.obs_length).toFixed(num_decimales) + add_post("m") +
            '</td><td>' + parseFloat(obs.sigma_total).toFixed(num_decimales) + add_post(obs.unit_str) + '</td>';

    if (obs.active)
        str+='<td class="redondance">' + parseFloat(obs.sigma_a_posteriori).toFixed(num_decimales) + add_post(obs.unit_str) + '</td>';
    else
        str+='<td class="redondance">-</td>';

    str+='<td>' + str_res_mm.toFixed(num_decimales_mini) + add_post("mm") +
            '</td><td>' + parseFloat(obs.normalized_residual).toFixed(num_decimales_mini) + add_post("𝜎") + '</td>';

    if (obs.active)
        str+='<td class="redondance">' + Math.abs(parseFloat(obs.obsRedondancy)).toFixed(num_decimales_mini-1) + add_post("%") +'</td>'
    else
        str+='<td class="redondance">-</td>';
    str+='</tr>';
...
function show_obs(){
  ...
  $("<a/>", {
    ...
    items = ["<table id='observations_table''> <thead><tr>" +
      ...
      "</th><th class='small' title='"+_('Écart entre l&apos;observation compensée et l&apos;observation mesurée') + "'>" + _("Résidu") +
      "</th><th class='small' title='"+_('Résidu divisé par le sigma total')+"' >" + _("Résidu normalisé") +
      "</th><th class='small redondance' title='"+_("0% : observation indispensable / 100% : observation inutile")+"'>" + _("Redondance") +
      "</th></tr></thead>"
    ];
    var last_file = -1;
...
function show_coord_comp(){
  ...
  for (var i = 0; i < all_pts.length; i++) {
    var pt = all_pts[i];
    items.push('<tr class=\"' + parite[i % 2] + '\" ><td class=\"name\">' + pt.name + add_post(codes_pt[pt.code]) +
      '</td><td>' + parseFloat(pt.coord_compensated_stereographic[0]).toFixed(num_decimales) +
...
function show_ellips(){
  ...
  for (var i = 0; i < all_pts.length; i++) {
    ...
    //Points 1D == Z
    {
      items.push('<tr class=\"' + parite[i % 2] + '\" ><td class=\"name\" >' + pt.name + add_post(codes_pt[pt.code]) + '</td><td>' +
      ...
      items.push('<tr class=\"' + parite[i % 2] + '\" ><td class=\"name\"  rowspan="3">' + pt.name + add_post(codes_pt[pt.code]) + '</td>');
      items.push('<td>' +
...
function show_intervalles(){
  ...
  for (var i = 0; i < all_pts.length; i++) {
    ...
    //Points 1D == Z
    if (pt.code === 4 || pt.code === 5) {
      items.push('<tr class=\"' + parite[i % 2] + '\"><td class=\"name\">' + pt.name + add_post(codes_pt[pt.code]) + '</td>');
...
function showComp3Djson(data) {
  ...
  show_info_work();
  show_info_calc();
  if (data["computation"]["all_sigma0"].length>0) {
    show_evol_sigma0();
  }
